...
echo '<div id="table" class="horzScroll">';
  ...
  echo '<div class="grid">';
	  ...
	  foreach( $items as $index => $item ) {
	  	...
	  	echo '<div class="click item droppable ' . $type . ' ' . $item->size() . '" data-index="' . $index . '" data-story="' . $story->slug() . '" data-slug="' . $item->slug() . '" data-type="' . $type . '" data-url="' . $item->url() . '">';
	  		echo '<div class="inner">';
				  snippet( 'items/' . $type, array( 'item' => $item ) );
				echo '</div>';
			echo '</div>';
		}
